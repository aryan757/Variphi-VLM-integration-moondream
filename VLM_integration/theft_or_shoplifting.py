...
class MoondreamService:
    ...
    def get_present_activities(self, image: Image.Image):
        ...
        prompt = (
        "Does this image show any act of theft or shoplifting? "
        "This includes someone secretly taking an item, hiding it, or stealing from another person or a store. "
        "If yes, respond with 'Yes' and a one-line description indicating the person involved and their location in the frame "
        "(e.g., 'The person on the top left is hiding an item under their jacket'). "
        "Use clear positional phrases like 'top left', 'bottom right', 'center', 'extreme right', etc. "
        "If there is no theft or shoplifting visible, respond with 'None'."
)
    ...
    def point_activities(self, image: Image.Image, get_present_activities: str):
        ...
        if not os.path.exists("theft_or_shoplifting_point_output"):
            os.makedirs("theft_or_shoplifting_point_output", exist_ok=True)
        ov.save(os.path.join("theft_or_shoplifting_point_output", filename))
    ...
    def detect_activities(self, image: Image.Image, get_present_activities: str):
        ...
        buf.seek(0)
        filename = f"detected_{random.randint(1, 1000)}.jpg"
        if not os.path.exists("theft_or_shoplifting_detect_output"):
            os.makedirs("behavioural_analytics_detect_output", exist_ok=True)
        ov.save(os.path.join("behavioural_analytics_detect_output", filename))
        print("filename", filename)
        print("boxes", boxes)
        return filename
...
def main():
    ...
    # process_videos("CCTV_real_data", output_dir="output_inference_6", frame_interval=100)
    # print("done")
