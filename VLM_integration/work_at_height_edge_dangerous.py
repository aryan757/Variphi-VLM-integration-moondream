...
class MoondreamService:
    ...
    def get_present_activities(self, image: Image.Image):
        ...
        prompt = (
            "Is there any person working in a dangerous condition, such as near an edge or at a dangerous height, in this image? "   
    ...
    def point_activities(self, image: Image.Image, get_present_activities: str):
        ...
        if not os.path.exists("construction_point_output"):
            os.makedirs("construction_point_output", exist_ok=True)
        ov.save(os.path.join("construction_point_output", filename))
    ...
    def detect_activities(self, image: Image.Image, get_present_activities: str):
        ...
        buf.seek(0)
        filename = f"detected_{random.randint(1, 1000)}.jpg"
        if not os.path.exists("construction_detect_output"):
            os.makedirs("construction_detect_output", exist_ok=True)
        ov.save(os.path.join("construction_detect_output", filename))
        print("filename", filename)
        print("boxes", boxes)
        return filename
