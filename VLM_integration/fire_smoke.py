...
class MoondreamService:
    ...
    def get_present_activities(self, image: Image.Image):
        ...
        # prompt = (
        #     "Does this image contain any of the following: fire or smoke ?"
        #     "Respond only the ones that are present, separated by commas."
        #     "If none are present, say 'none'."
        # )

        ## write it in the same way as you have written it for phone_usage.py file , here write the prompt for fire and smoke
        prompt = (
            "Does this image contain any of the following: fire or smoke ?"
        ...
        if "Yes" or "yes" and "fire" or "smoke" or "Fire" or "Smoke" in response_text.lower():
            return "fire" + " " + "smoke" + " " + "Fire" + " " + "Smoke"
    ...
    def point_activities(self, image: Image.Image, get_present_activities: str):
        ...
        if not os.path.exists("fire_smoke_point_output"):
            os.makedirs("fire_smoke_point_output", exist_ok=True)
        ov.save(os.path.join("fire_smoke_point_output", filename))
    ...
    def detect_activities(self, image: Image.Image, get_present_activities: str):
        ...
        buf.seek(0)
        filename = f"detected_{random.randint(1, 1000)}.jpg"
        if not os.path.exists("fire_smoke_detect_output"):
            os.makedirs("fire_smoke_detect_output", exist_ok=True)
        ov.save(os.path.join("fire_smoke_detect_output", filename))
        print("filename", filename)
        print("boxes", boxes)
        return filename
