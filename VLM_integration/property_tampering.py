...
class MoondreamService:
    ...
    def point_activities(self, image: Image.Image, get_present_activities: str):
        ...
        if not os.path.exists("property_tampering_point_output"):
            os.makedirs("property_tampering_point_output", exist_ok=True)
        ov.save(os.path.join("property_tampering_point_output", filename))
    ...
    def detect_activities(self, image: Image.Image, get_present_activities: str):
        ...
        buf.seek(0)
        filename = f"detected_{random.randint(1, 1000)}.jpg"
        if not os.path.exists("property_tampering_detect_output"):
            os.makedirs("property_tampering_detect_output", exist_ok=True)
        ov.save(os.path.join("property_tampering_detect_output", filename))
        print("filename", filename)
        print("boxes", boxes)
        return filename
