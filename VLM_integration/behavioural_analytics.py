...
class MoondreamService:
    ...
    def get_present_activities(self, image: Image.Image):
        ...
        """
        prompt = (
            "Does this image show any form of assault or physical attack between people? "
            "If yes, respond with 'Yes' followed by a one-liner clearly describing the person involved and their approximate position in the frame. "
            "Use location terms like 'the person on the top left', 'center', 'extreme right', etc. "
    ...
    def point_activities(self, image: Image.Image, get_present_activities: str):
        ...
        if not os.path.exists("behavioural_analytics_point_output"):
            os.makedirs("behavioural_analytics_point_output", exist_ok=True)
        ov.save(os.path.join("behavioural_analytics_point_output", filename))
    ...
    def detect_activities(self, image: Image.Image, get_present_activities: str):
        ...
        buf.seek(0)
        filename = f"detected_{random.randint(1, 1000)}.jpg"
        if not os.path.exists("behavioural_analytics_detect_output"):
...
def main():
    ...
    # process_images("image-dataset", output_dir="output-dir")
